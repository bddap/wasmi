...
impl ValueStack {
    ...
    fn drop_keep(&mut self, drop_keep: isa::DropKeep) {
        match drop_keep.keep {
            isa::Keep::Single => {
                ...
                let pick = self.pick_mut(drop_keep.drop as usize + 1);
                *pick = top;
            }
            isa::Keep::None => {}
        };
    ...
    fn top(&self) -> &RuntimeValue {
        self.stack
            .top()
            .expect("This function is unsafe. We are implicitly assuming the stack is not empty.")
    }
    ...
    fn pick_mut(&mut self, depth: usize) -> &mut RuntimeValue {
        ...
        self.stack
            .get_mut(len - depth)
            .expect("This function may be unsafe. We are assuming len() != 0, and depth <= len().")
    }
    ...
    fn pop(&mut self) -> RuntimeValue {
        self.stack
            .pop()
            .expect("We are assuming that pop will never be called on an empty ValueStack.")
    }
    ...
    fn drop_many(&mut self, count: usize) {
        debug_assert!(
            count <= self.len(),
            "Attempted to drop more items than were in stack."
        );
        let new_len = if count > self.stack.len() {
            0
        } else {
            self.len() - count
        };
